...
def calc_rr_gbd2019_pm25(pm25, dict_pm25):
    """
    convert PM2.5 to rr using the GBD2019 IER function for PM2.5
    call example: calc_rr_gbd2019_pm25(pm25, dict_pm25)
    """
    ...
    for cause in ['lri', 'lc', 'copd', 'diab']:
        for uncertainty in uncertainties:
            dict_rr.update({
                ...
                np.array([[dict_pm25['rr_gbd2019_' + cause + '_' + uncertainty][find_nearest(dict_pm25['rr_gbd2019_conc'], pm25[lat][lon])]
                for lon in range(pm25.shape[1])] for lat in range(pm25.shape[0])])
            })
        ...
        for uncertainty in uncertainties:
            for age in ages:
                dict_rr.update({
                    ...
                    np.array([[dict_pm25['rr_gbd2019_' + cause + '_' + uncertainty + '_' + age][find_nearest(dict_pm25['rr_gbd2019_conc'], pm25[lat][lon])]
                    for lon in range(pm25.shape[1])] for lat in range(pm25.shape[0])])
                })
...
def calc_hia_gbd2019_pm25(pm25, dict_rr_pm25, pop_z_2015, dict_ages, dict_bm, dict_pm25):
    """
    ...
    call example: calc_hia_gbd2019_pm25(pm25, dict_rr_pm25, pop_z_2015, dict_ages, dict_bm, dict_pm25)
    """
    # inputs
    causes = ['lri', 'lc', 'copd', 'diab', 'str', 'ihd']
    ages = ['25_29', '30_34', '35_39', '40_44', '45_49', '50_54', '55_59', '60_64', '65_69', '70_74', '75_79', '80_84', '85_89', '90_94', '95up']
    ...
    hia_pm25.update({ 'pm25_popweighted' : pop_z_2015 * pm25 })
            ...
            for uncertainty in uncertainties:
                for age in ages:
                    hia_pm25.update({
                        ...
                        * (dict_rr_pm25[cause + '_' + uncertainty] - 1) / dict_rr_pm25[cause + '_' + uncertainty]
                    })
    ...
    for measure in measures:
        for uncertainty in uncertainties:
            hia_pm25.update({
                ...
                * (dict_rr_pm25['lri_' + uncertainty] - 1) / dict_rr_pm25['lri_' + uncertainty]
            })
            ...
            for uncertainty in uncertainties:
                for age in ages:
                    hia_pm25.update({
                        ...
                        * dict_ages['age_fraction_' + uncertainty + '_' + age + '_both']
                        * dict_bm['i_' + measure + '_' + cause + '_both_' + uncertainty + '_' + age]
                        * (dict_rr_pm25[cause + '_' + uncertainty + '_' + age] - 1) / dict_rr_pm25[cause + '_' + uncertainty + '_' + age]
                    })
            ...
            for uncertainty in uncertainties:
                for age in ages:
                    hia_pm25.update({
                        ...
                        * dict_ages['age_fraction_' + uncertainty + '_' + age + '_both']
                        * dict_bm['i_' + measure + '_' + cause + '_both_' + uncertainty + '_' + age]
                        * ((( (ratio[cause] * dict_rr_pm25[cause + '_' + uncertainty + '_' + age]) - ratio[cause] + 1 ) - 1) / ( (ratio[cause] * dict_rr_pm25[cause + '_' + uncertainty + '_' + age]) - ratio[cause] + 1 ))
                    })
        ...
        for measure in measures:
            for uncertainty in uncertainties:
                hia_pm25.update({
                    ...
                    sum([value for key, value in hia_pm25.items() if measure + '_' + cause + '_' + uncertainty in key])
                })
        ...
        for measure in measures:
            for uncertainty in uncertainties:
                hia_pm25.update({
                    ...
                    hia_pm25['yll_' + cause + '_' + uncertainty + '_total'] + hia_pm25['yld_' + cause + '_' + uncertainty + '_total']
                })
        ...
        for uncertainty in uncertainties:
            ...
            hia_pm25.update({
                ...
                sum([value for key, value in hia_pm25.items()
                    if measure in key and uncertainty in key
                    and not '6cod' in key])
            })
            # 6cod rates - total
            hia_pm25.update({
                measure + '_rate_6cod_' + uncertainty + '_total' :
                hia_pm25[measure + '_6cod_' + uncertainty + '_total'] * ( 100000 / pop_z_2015)
            })
